...
for (i in cluster.number) {
  ...
  (gp <- ggplot() +
      ...
      geom_line(data = res.gp[variable %in% unlist(disc),], aes(x = gumbel.variate, y = scaled.value, group = variable), lty = 'dashed', colour = 'grey5', na.rm = T) +
      geom_point(data = res.gp[variable %in% unlist(disc),], aes(x = gumbel.variate, y = scaled.value, group = variable), colour = 'grey5', fill = NA, shape = 21, na.rm = T) +
  ...
  (discgg <- ggplot(data = NULL, aes(x = m_3, y = m_2/m_1)) +
      ...
      geom_point(data = moments[rn %in% unlist(disc),], colour = 'grey15', shape = 13, size = 7.5) +
      theme_bw() + 
      scale_y_continuous(labels = function(x) sprintf("%.2f", x)) +
      scale_x_continuous(labels = function(x) sprintf("%.2f", x)) +
      labs(x = 'L-skewness', y = 'L-CV', subtitle = 'Discordance') +
      theme(aspect.ratio = 1))
  
  cl <- grid.arrange(gp, discgg, nrow = 1, widths = c(1.75, .95))
  
  # ggsave(paste0('~/Desktop/cl',i,'.pdf'), cl, width = 9, height = 3, units = 'in')
}
