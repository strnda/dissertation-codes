...
for(i in 1:length(val)){
  ...
  gg <- ggplot(dta) +
    geom_line(aes(x = P, y = MEAN, group = factor(VAR), colour = factor(VAR)), size = 0.5) +
    geom_ribbon(aes(x = P,ymin = Q25, ymax = Q75, fill = factor(VAR), colour = factor(VAR)), alpha = 0.25) +
    scale_fill_manual(values = c('steelblue4', 'red4'), labels = c('Observed values', 'Simulated values'), name = NULL) +
    scale_colour_manual(values = c('steelblue4', 'red4'), labels = c('Observed values', 'Simulated values'), name = NULL) +
