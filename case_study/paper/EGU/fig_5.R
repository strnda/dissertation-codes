...
(adgg <- ggplot(polygony) +
    geom_boxplot(aes(x = factor(cluster), y = p, group = cluster, fill = factor(cluster)), width = .8) +
    ...
    scale_fill_manual(values = c('gold4','orange','red4'), guide = F) +
    theme_bw() +
...
(paragg <- ggplot(para.m) +
    geom_boxplot(aes(x = factor(cluster), y = value, group = cluster, fill = factor(cluster)), outlier.size = 1, width = 2.5) +
...
# ggsave('~/Desktop/boxplot.pdf', xxx)
